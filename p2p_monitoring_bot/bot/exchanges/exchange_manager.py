...
class ExchangeManager:
    ...
    async def get_combined_offers(
        self, exchange_names: Optional[List[str]] = None, force_refresh: bool = False
    ) -> List[Dict[str, Any]]:
        """
            ...
            for exchange_name, result in zip(task_names, results):
                try:
                    ...
                    if offers:
                        ...
                        logger.info(f"✅ Got {len(offers)} offers from {exchange_name}")
                    else:
        ...
        logger.info(
            f"Combined {len(combined_offers)} offers from {len(exchange_names)} exchanges"
        )
